...
/**
 * vfs_iterate_dir - 遍历指定 dentry 对应的目录
 * @dentry:  目录 dentry
 * @dirent:  传给 filldir 的上层上下文指针
 * @filldir: 回调函数，由上层决定如何处理目录项
 *
 * 返回值：0 表示成功，负值表示错误。
 */
int vfs_iterate_dir(struct dentry *dentry, void *dirent, filldir_t filldir)
{
	...
	fops = inode->i_fop;
	if (!fops || !fops->readdir) {
		return -3;
	}

	/* 简化版 file 初始化，只设置遍历目录所需的字段 */
	file.f_inode       = inode;
	file.f_path_dentry = dentry;
	file.f_op          = fops;
	file.f_pos         = 0;
	file.f_flags       = 0;
	file.f_mode        = 0;
	...
	return fops->readdir(&file, dirent, filldir);
}
...
int vfs_ls(const char *path, void *dirent, filldir_t filldir)
{
	...
	return vfs_iterate_dir(target, dirent, filldir);
}
